...
- (void)viewDidLoad
{
    ...
    //通知
    DIEAddObserver(self, @selector(didUpdate:), kDIEAnimeUpdateNotif, nil);
    [[DIEDataManeager sharedManager] updateCategory];
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section
{
    ...
    switch (section) {
        case 0:
            number = [[DIEDataManeager sharedManager].categoriesArray count];
            break;
        default:
            number = 0;
            break;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    DIECategoryModel *cellModel = [DIEDataManeager sharedManager].categoriesArray[indexPath.item];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(nonnull NSIndexPath *)indexPath
{
    ...
    NSString *categoryId = [DIEDataManeager sharedManager].categoriesArray[indexPath.item].categoryId;
    DIEAnimeViewController *animeViewControllr = [DIEAnimeViewController new];
    animeViewControllr.categoryId = categoryId;
    animeViewControllr.title = cell.textLabel.text;
    [[DIEDataManeager sharedManager].animeArray removeAllObjects];
    [self.navigationController pushViewController:animeViewControllr animated:YES];
    
    
    
    //跳转
//    [self.navigationController pushViewController:[[DIEViewController alloc] init] animated:YES];
}
@end
